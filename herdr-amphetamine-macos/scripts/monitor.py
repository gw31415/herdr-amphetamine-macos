...
def get_agent_statuses(herdr_bin: str) -> list:
    """Run `herdr agent list` and return the list of agent_status strings.

    herdr prints a JSON envelope: {"id":..., "result":{"agents":[{...}], ...}}.
    ...
    try:
        ...
        raise HerdrError("herdr agent list timed out") from exc
    if proc.returncode != 0:
        raise HerdrError(
            f"herdr agent list exited {proc.returncode}: {(proc.stderr or '').strip()}"
        )
    try:
        data = json.loads(proc.stdout)
    except ValueError as exc:
        raise HerdrError(f"could not parse herdr output as JSON: {exc}") from exc
...
class Config:
    ...
    stop_grace: float
    state_path: Path
    session_minutes: float = DEFAULT_SESSION_MINUTES
    extend_threshold_minutes: float = DEFAULT_EXTEND_THRESHOLD_MINUTES
    armed: bool = True
...
def load_config() -> Config:
    ...
    return Config(
        ...
        session_minutes=raw["session_minutes"],
        extend_threshold_minutes=raw["extend_threshold_minutes"],
        armed=raw["armed"],
...
def iterate(cfg: Config, ctx: MonitorCtx, now: float) -> MonitorCtx:
    ...
    def _start():
        amphetamine_ctl.start_session(cfg.display_sleep_allowed, cfg.session_minutes)
...
def print_status(cfg: Config) -> None:
    ...
    print(f"session length:  {cfg.session_minutes:g}m{' (infinite)' if cfg.session_minutes == 0 else ''} "
          f"(extend at {cfg.extend_threshold_minutes:g}m)")
...
def run_daemon(initial_cfg: Config) -> None:
    ...
    log(f"Monitor started (poll={cfg.poll_seconds}s start_grace={cfg.start_grace}s "
        f"stop_grace={cfg.stop_grace}s session={cfg.session_minutes:g}m "
        f"extend_at={cfg.extend_threshold_minutes:g}m armed={cfg.armed} "
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="Resident Amphetamine sleep-guard daemon for herdr."
    )

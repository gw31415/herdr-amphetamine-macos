...
def render(stdscr, d, footer_msg):
    ...
    stdscr.addnstr(y, max(0, w - len(badge_text) - 2), badge_text,
                   len(badge_text), badge_attr | curses.A_BOLD)
    y += 1
    stdscr.addnstr(y, 0, bar, w); y += 1
    ...
    if y < h:
        stdscr.addnstr(y, 0, " Settings    (press a key to edit; Space = start/pause)",
                       w, curses.A_BOLD); y += 1
    ...
    for label, value, attr in settings_rows:
        if y >= h - 2:
            break
        _row(stdscr, y, label, value, attr)
        y += 1

    if y < h:
        stdscr.addnstr(y, 0, dash, w); y += 1
    if y < h:
        stdscr.addnstr(y, 0, f" Recent  ({LOG_PATH})", w, curses.A_BOLD); y += 1
    if d["tail"]:
        for line in d["tail"]:
            if y >= h - 2:
                break
    ...
    if y < h:
        stdscr.addnstr(y, 0, bar, w); y += 1
    keys = "[Space]start/pause  [i]install  [u]ninstall  [r]efresh  [?]help  [q]uit"
    stdscr.addnstr(h - 1, 0, keys.ljust(w)[:w], w)
    if footer_msg:
        stdscr.addnstr(h - 1, max(0, w - len(footer_msg) - 1),
                       footer_msg, len(footer_msg), _attr("yellow"))
    stdscr.refresh()
